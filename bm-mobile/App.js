...
function ExpensesScreen({ token, buildingId, expenses, reload }) {
  ...
  const saveExpense = async () => {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(editDate)) return Alert.alert('تنبيه', 'أدخل التاريخ بصيغة 2026-01-01');
...
function OwnersScreen({ token, buildingId, owners, reload }) {
  ...
  const startEdit = (owner) => { setEditingOwner(owner); setForm({ name: owner.name || '', phone: owner.phone || '', national_id: owner.national_id || owner.login || '', email: owner.email || '', apartmentsText: (owner.apartments || []).join('، '), notes: owner.notes || '' }); setOwnerFormVisible(true); };
...
function BuildingSettingsScreen({ token, buildingId, dashboard, reload, setTab }) {
  ...
  const [tempYear, setTempYear] = useState(String((annualCycleStartsOn || '2026-01-01').slice(0, 4)));
  const [tempMonth, setTempMonth] = useState(String(Number((annualCycleStartsOn || '2026-01-01').slice(5, 7)) || 1));
  const [tempDay, setTempDay] = useState(String(Number((annualCycleStartsOn || '2026-01-01').slice(8, 10)) || 1));
  const [loading, setLoading] = useState(false);
  ...
  const openDatePicker = () => {
    const current = annualCycleStartsOn || '2026-01-01';
...
function OwnerOnlyScreen({ token }) { const [data, setData] = useState(null); const [loading, setLoading] = useState(true); useEffect(() => { request('/owner/dashboard', {}, token).then(setData).catch((e) => Alert.alert('خطأ', e.message)).finally(() => setLoading(false)); }, [token]); if (loading) return <LoadingScreen />; const profile = data?.owners?.[0]; if (!profile) return <EmptyState icon="home-outline" title="لا توجد بيانات" text="لم يتم ربط حسابك بمالك بعد." />; return <ScrollView contentContainerStyle={styles.screenContent}><ScreenCode code="#S-009" />
      <Dashboard dashboard={{ building: profile.building, stats: {}, owners: [profile.summary] }} /><SectionTitle icon="receipt-outline" title="تفصيل نصيبك من المصروفات" />{(profile.expenses || []).map((item) => <ExpenseRow key={item.id} item={{ ...item, amount: item.owner_share }} />)}</ScrollView>; }
...
function AppShell({ token, user, selectedBuilding, setSelectedBuilding, onLogout }) {
  const [tab, setTab] = useState('dashboard'); const [dashboard, setDashboard] = useState(null); const [expenses, setExpenses] = useState([]); const [payments, setPayments] = useState([]); const [loading, setLoading] = useState(true);
  const reload = async () => { if (!selectedBuilding) return; setLoading(true); try { const [dash, expenseData, paymentData] = await Promise.all([request(`/buildings/${selectedBuilding.id}/dashboard`, {}, token), request(`/buildings/${selectedBuilding.id}/expenses`, {}, token), request(`/buildings/${selectedBuilding.id}/payments`, {}, token)]); setDashboard(dash); setExpenses(expenseData.data || []); setPayments(paymentData.data || []); } catch (e) { Alert.alert('تعذر تحميل البيانات', e.message); } finally { setLoading(false); } };
  ...
  return <SafeAreaView style={styles.container}><Header title={selectedBuilding?.name || 'المبنى'} subtitle="إدارة اتحاد الملاك" onLogout={onLogout} onBack={() => setSelectedBuilding(null)} />{loading ? <LoadingScreen /> : <>{tab === 'dashboard' && <Dashboard dashboard={dashboard} />}{tab === 'owners' && <OwnersScreen token={token} buildingId={selectedBuilding.id} owners={owners} reload={reload} />}{tab === 'expenses' && <ExpensesScreen token={token} buildingId={selectedBuilding.id} expenses={expenses} reload={reload} />}{tab === 'payments' && <PaymentsScreen token={token} buildingId={selectedBuilding.id} owners={owners} payments={payments} reload={reload} />}{tab === 'settings' && <SettingsScreen dashboard={dashboard} setTab={setTab} />}{tab === 'buildingSettings' && <BuildingSettingsScreen token={token} buildingId={selectedBuilding.id} dashboard={dashboard} reload={reload} setTab={setTab} />}</>}<View style={styles.tabs}><TabButton active={tab === 'dashboard'} icon="grid-outline" title="الملخص" onPress={() => setTab('dashboard')} /><TabButton active={tab === 'owners'} icon="people-outline" title="الملاك" onPress={() => setTab('owners')} /><TabButton active={tab === 'expenses'} icon="receipt-outline" title="المصروفات" onPress={() => setTab('expenses')} /><TabButton active={tab === 'payments'} icon="wallet-outline" title="الدفعات" onPress={() => setTab('payments')} /><TabButton active={tab === 'settings' || tab === 'buildingSettings'} icon="settings-outline" title="الإعدادات" onPress={() => setTab('settings')} /></View></SafeAreaView>;
}
...
const styles = StyleSheet.create({
  ...
  ownerCard: { backgroundColor: '#fff', borderRadius: 22, padding: 14, borderWidth: 1, borderColor: '#e2e8f0' }, ownerTop: { flexDirection: 'row-reverse', alignItems: 'center', gap: 10 }, ownerAvatar: { width: 42, height: 42, borderRadius: 16, backgroundColor: '#ecfdf5', alignItems: 'center', justifyContent: 'center' }, badge: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999 }, badgeDue: { backgroundColor: '#fee2e2' }, badgeSurplus: { backgroundColor: '#dcfce7' }, badgeBalanced: { backgroundColor: '#e0f2fe' }, badgeText: { color: '#0f172a', fontWeight: '800', fontSize: 11 }, ownerAmounts: { flexDirection: 'row-reverse', gap: 8, marginTop: 12, flexWrap: 'wrap' }, smallAmount: { flexGrow: 1, minWidth: '45%', backgroundColor: '#f8fafc', borderRadius: 16, padding: 10 }, smallTitle: { fontSize: 11, color: '#64748b', textAlign: 'right' }, smallValue: { fontSize: 13, color: '#0f172a', fontWeight: '900', textAlign: 'right', marginTop: 4 },
  ...
  manageOwnerCard: { marginBottom: 12, backgroundColor: '#fff', borderRadius: 24, padding: 10, borderWidth: 1, borderColor: '#e2e8f0' }, ownerMetaRow: { flexDirection: 'row-reverse', gap: 8, flexWrap: 'wrap', paddingHorizontal: 4, paddingTop: 8 }, ownerMeta: { fontSize: 11, color: '#64748b', textAlign: 'right' }, actionsRow: { flexDirection: 'row-reverse', gap: 8, marginTop: 10 }, actionBtn: { flex: 1, height: 42, borderRadius: 14, backgroundColor: '#ecfdf5', alignItems: 'center', justifyContent: 'center', flexDirection: 'row-reverse', gap: 6 }, deleteBtn: { backgroundColor: '#fef2f2' }, actionText: { color: '#0f766e', fontWeight: '900', fontSize: 13 }, deleteText: { color: '#ef4444' }, ownerFloatingAdd: { position: 'absolute', top: 10, left: 16, width: 54, height: 54, borderRadius: 27, backgroundColor: '#0f766e', alignItems: 'center', justifyContent: 'center', shadowColor: '#0f172a', shadowOpacity: 0.18, shadowRadius: 14, elevation: 8, zIndex: 10 },
