...
function ExpensesScreen({ token, buildingId, expenses, categories, reload }) {
  ...
  const [expenseFormVisible, setExpenseFormVisible] = useState(false); const [categoryLocked, setCategoryLocked] = useState(false); const [category, setCategory] = useState(options[0]); const [amount, setAmount] = useState(''); const [expenseDate, setExpenseDate] = useState(todayDate()); const [description, setDescription] = useState(''); const [loading, setLoading] = useState(false);
  ...
  const closeAddExpenseForm = () => { resetAddExpenseForm(); setCategoryLocked(false); setExpenseFormVisible(false); };
  const openAddExpenseForCategory = (categoryName) => { setCategory(categoryName); setCategoryLocked(true); setAmount(''); setExpenseDate(todayDate()); setDescription(''); setExpenseFormVisible(true); };
  const openGeneralAddExpense = () => { setCategoryLocked(false); resetAddExpenseForm(); setExpenseFormVisible(true); };
  const openAddFromSelectedCategory = () => { const categoryName = selectedExpenseCategory?.category; if (!categoryName) return; setExpenseCategoryDetailsVisible(false); openAddExpenseForCategory(categoryName); };
