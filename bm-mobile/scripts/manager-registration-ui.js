...
const replacement = `function ManagerRegistrationScreen({ onBack, onLogin }) {
  ...
  const requestOtp = async () => {
    ...
    if (!form.phone.trim()) return Alert.alert('تنبيه', 'أدخل رقم الجوال');
    try {
      setLoading(true);
      ...
      setStep('otp');
      Alert.alert('رمز التحقق', data.message || 'تم إرسال رمز التحقق على رقم الجوال.');
    } catch (e) {
