...
const logoutTitle = 'خروج';
const logoutText = 'تسجيل الخروج من الحساب';
...
if (!content.includes(`title="${logoutTitle}" text="${logoutText}"`)) {
  content = content.replace(
    `onPress={() => setTab('buildingSettings')} /></ScrollView>; }`,
    `onPress={() => setTab('buildingSettings')} /><SettingsLink icon="log-out-outline" title="${logoutTitle}" text="${logoutText}" onPress={globalThis.__bmLogout || (() => {})} /></ScrollView>; }`
  );
  content = content.replace(
    `<SettingsScreen dashboard={dashboard} setTab={setTab} />`,
    `<SettingsScreen dashboard={dashboard} setTab={setTab} onLogout={onLogout} />`
  );
  content = content.replace(
    `function SettingsScreen({ dashboard, setTab })`,
    `function SettingsScreen({ dashboard, setTab, onLogout })`
  );
  content = content.replace(
    `onPress={globalThis.__bmLogout || (() => {})}`,
    `onPress={onLogout}`
  );
  applied += 1;
}
